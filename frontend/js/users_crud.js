document.addEventListener("DOMContentLoaded", function() {
    ...
    createUserBtn.addEventListener("click", function(event) {
        ...
        if (validateForm()) {
            ...
            let formData = {};
            $(userForm).serializeArray().forEach(item => {
    ...
    function validateForm() {
        ...
        let isValid = true;
        ['firstName', 'lastName', 'role', 'idNumber', 'phone', 'email', 'password', 'confirmPassword'].forEach(inputName => {
            let inputElement = document.getElementById(inputName);
...
function deleteUser(recordId) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '../../../backend/functions/users_crud.php',
                method: 'POST',
                dataType: 'json',
                data: {
                    deletePerson: true,
...
function displayUsers(users) {
    ...
    $.each(users, function(index, user) {
        ...
        let newRow = '<tr>' +
            ...
            '<td>' + user.MainContactNumber + '</td>' +
            '<td>' + user.AccessBlocked + '</td>' +
            '<td>' + user.UserRole + '</td>' +
            '<td>' +
            '<button class="btn btn-secondary m-1" onclick="editUser('+user.Id+')">Edit</button>' +
            '<button class="btn btn-warning m-1" onclick="deleteUser('+user.Id+')">Delete</button>' +
            '</td>' +
            '</tr>';
